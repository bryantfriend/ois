...
## Geography adventures (13 September 2026)

All 116 activities cover the original 58 topics and grade groups, with 29 activities per player mode. Each contains two missions using ten shared families: atlas reconstruction, compass expeditions, flood-defence simulation, settlement planning, supply-chain routing, water-drop journeys, landscape sculpting, evidence creation, museum sequences and fieldwork sampling. No Geography activity uses answer-choice questions. Legacy saved/imported Geography lessons become creation briefs while preserving their source and model answer; storage is changed only when saved.

The collection retains original SVG worlds and characters. Atlas outlines use public-domain Natural Earth data; fictional grids, datasets and simplified models are identified in mission briefs. Creation studios need teacher/partner review of geographical reasoning. Most controls support tap and keyboard; freehand drawing requires a pointer. Two-player panels support simultaneous touch; Teams choose 2–6 names and lock work before shared review.

Teacher research sources, limitations, original ideas and all playable adaptations: [Geography teacher guide](dist/geography-guide.html). Rebuild with `node scripts/build-geography-guide.mjs`. Atlas geometry is generated by `scripts/build-geography-atlas.mjs`; gameplay makes no external data requests.

Run `qa-geography-adventures.mjs`, `qa-geography-adventure-layout.mjs` and `qa-geography-adventure-edges.mjs` from scripts for catalogue completion, layout and native interaction/persistence checks. Set `OXFORD_PLAYWRIGHT_MODULE` to your Playwright module path. These supersede the earlier Geography/mission suites, which describe previous releases. Run `npm run check` and `node scripts/version-assets.mjs` before publishing so browser asset URLs reflect content changes.
