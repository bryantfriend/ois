...
function renderLibrary(){
 ...
 const filtered=source.filter(g=>(ui.grade==='all'||g.grade===ui.grade)&&(ui.subject==='all'||g.subject===ui.subject)&&(ui.players==='all'||playerModes(g.format).includes(ui.players))&&`${g.title} ${g.topic} ${FORMATS[g.format].name} ${FORMATS[g.format].sourceTitle||''} ${LAB_GAMES[g.format]?'PlayMath':''} ${window.OXFORD_WORLDS?.[g.format]?.name||""}`.toLowerCase().includes(ui.search.toLowerCase()));
