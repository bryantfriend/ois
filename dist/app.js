...
function validateLesson(raw){
 ...
 const items=raw.items.map((item,i)=>{
  ...
  return {prompt,answer,options,hint:item.hint||'',explanation:item.explanation||'',...activityItemFields(raw.format,item,i)};
 });
 validateActivityLesson(raw,items);
 ...
 const settings={...activitySettings(raw.settings),teamOne:String(raw.settings?.teamOne||'Player 1').slice(0,30),teamTwo:String(raw.settings?.teamTwo||'Player 2').slice(0,30),shuffle:raw.settings?.shuffle===true};
 Object.assign(settings,boozledSettings(raw.settings));
...
function renderLibrary(){
 ...
 const filtered=source.filter(g=>(ui.grade==='all'||g.subject==='ice-breakers'||g.grade===ui.grade)&&(ui.subject==='all'||g.subject===ui.subject)&&(ui.players==='all'||playerModes(g.format).includes(ui.players))&&`${g.title} ${g.topic} ${FORMATS[g.format].name} ${FORMATS[g.format].sourceTitle||''} ${LAB_GAMES[g.format]?'PlayMath':''} ${window.OXFORD_WORLDS?.[g.format]?.name||""}`.toLowerCase().includes(ui.search.toLowerCase()));
 ...
 $('#games').innerHTML=filtered.map(g=>`<article class="game-card ${g.subject==='math'?'math':'english'}"><div class="card-art art-${g.format}" aria-hidden="true"><span class="art-symbol">${FORMATS[g.format].icon}</span><span class="art-mini">${g.subject==='math'?'x + y':'Aa'}</span><span class="format-tag">${FORMATS[g.format].name}</span></div><div class="card-body"><div class="card-meta"><span>${esc(SUBJECTS.find(s=>s.id===g.subject).name)}</span><span>${g.subject==='ice-breakers'?'ALL GRADES':'GRADE '+g.grade}</span></div><h3>${esc(g.title)}</h3><p>${esc(g.topic)}</p>${g.subject==='geography'?`<span class="gx-release-label">NEW · ${g.items.map(q=>esc(GX_TYPES[q.geo?.kind?.slice(3)]||'Creation studio')).join(' + ')}</span>`:''}<div class="player-tags">${playerModes(g.format).map(mode=>`<span>${PLAYER_LABELS[mode]}</span>`).join('')}</div><div class="card-details"><span>◷ ${g.minutes} min</span><span>${g.items.length} ${GEO_FORMATS[g.format]?'missions':LAB_GAMES[g.format]?'challenges':g.format==='match'?'pairs':'questions'}</span></div><div class="card-actions"><button class="edit-card" data-edit="${esc(g.id)}">Edit & play <span>↗</span></button><button class="quick-play" data-play="${esc(g.id)}" aria-label="Play ${esc(g.title)} grade ${g.grade}">▶</button></div></div></article>`).join('');
...
$('#games').onclick=e=>{const b=e.target.closest('[data-edit],[data-play]');if(!b)return;const lesson=(ui.collection==='saved'?saved:GAMES).find(g=>g.id===(b.dataset.edit||b.dataset.play));if(!lesson)return;openEditor(lesson);if(FORMATS[lesson.format].mode==='TEAM')openTeamSetup();else if(b.dataset.play)startGame();};
function openEditor(lesson){
 ...
 $('#team-fields').hidden=!(['tug','relay','board'].includes(draft.format)||LAB_GAMES[draft.format]?.mode==='DUEL'||GEO_FORMATS[draft.format]?.mode==='DUEL');
 setupModeEditor();
...
$('#import-file').onchange=async e=>{const file=e.target.files[0];if(!file)return;try{if(file.size>6000000)throw Error('Choose a lesson file smaller than 6 MB.');const data=JSON.parse(await file.text());if(data.schemaVersion!==1)throw Error('This is not a supported Oxford lesson file.');const lesson=validateLesson(data.lesson);lesson.id=`custom-${crypto.randomUUID()}`;openEditor(lesson);dirty=true;notify('Lesson opened. Review it, then play or save a copy.');}catch(error){notify(error instanceof SyntaxError?'The file is not valid JSON. Choose an Oxford lesson download.':error.message);}e.target.value='';};
window.addEventListener('beforeunload',e=>{if(dirty){e.preventDefault();e.returnValue='';}});
function startGame(skipBoozledSetup=false){if(draft?.format==='boozled'&&skipBoozledSetup!==true){openBoozledSurprises(true);return;}validAction(lesson=>{if(['tug','relay','board'].includes(lesson.format)&&lesson.items.some(q=>!q.options.length)){notify('Add at least one wrong choice to every question for this two-player game.');return;}draft=lesson;game={lesson:clone(lesson),items:lesson.settings.shuffle?shuffle(lesson.items):clone(lesson.items),index:0,score:lesson.format==='wager'?300:0,teams:[0,0],turn:0,rope:0,lives:3,streak:0,answered:false,revealed:false,hinted:false,wager:0,done:false,correct:0,attempts:0,history:[],used:[],selected:[],left:null,matched:[],feedback:'',feedbackGood:false};game.right=shuffle(game.items.map((_,i)=>i));if(['tug','relay','board'].includes(lesson.format))initTug();prepareRound();if(ACTIVITIES[lesson.format]||EXPANSION[lesson.format]||['kingdom','corners','earth','boozled'].includes(lesson.format))initModeGame();show('player');renderGame();});}
...
$("#lab-browse").onclick=()=>{ui.collection="library";resetFilters();ui.search="PlayMath";$("#search").value=ui.search;renderLibrary();$("#library-title").scrollIntoView({behavior:"smooth",block:"start"});};
