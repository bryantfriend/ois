...
try{
 ...
 assert.equal(await page.evaluate(()=>GAMES.map(validateLesson).length),176);
 ...
 assert.equal(toolResult.name,'filter_classroom_games');assert.equal(toolResult.result.summary,'42 lessons · Grade 8');assert.equal(toolResult.visible,'42 lessons · Grade 8');assert.equal(toolResult.invalid,true);await toolsContext.close();
