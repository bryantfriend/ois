...
try{
 ...
 const tugAnswer=await page.evaluate(()=>game.tug[0].choices.indexOf(game.items[game.tug[0].question].answer));await page.locator(`[data-player="0"][data-tug-answer="${tugAnswer}"]`).click();await page.evaluate(()=>window.advanceTime(200));const hit=await page.evaluate(()=>JSON.parse(render_game_to_text()));assert.equal(hit.animation.event,'hit');assert.ok(hit.animation.particles>0);assert.ok(hit.animation.ropePosition<0);assert.equal(hit.rope,-1);await page.screenshot({path:'output/arcade/tug-hit.png'});
