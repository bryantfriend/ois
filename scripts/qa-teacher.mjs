...
try{
 ...
 assert.equal(await page.evaluate(()=>GAMES.map(validateLesson).length),52);
 ...
 assert.equal(toolResult.name,'filter_classroom_games');assert.equal(toolResult.result.summary,'13 lessons · Grade 8');assert.equal(toolResult.visible,'13 lessons · Grade 8');assert.equal(toolResult.invalid,true);await toolsContext.close();
