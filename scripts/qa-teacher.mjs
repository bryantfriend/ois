...
try{
 ...
 assert.equal(await page.evaluate(()=>GAMES.map(validateLesson).length),490);
 ...
 assert.equal(toolResult.name,'filter_classroom_games');assert.equal(toolResult.result.summary,'51 lessons · Grade 8');assert.equal(toolResult.visible,'51 lessons · Grade 8');assert.equal(toolResult.invalid,true);await toolsContext.close();
