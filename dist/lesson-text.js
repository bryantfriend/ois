...
function paint(){
 panel.hidden=!draft||draft.subject==='board-games';
 if(!draft)return;
 $('#lesson-text-content').hidden=!textMode;
 $('#lesson-editor-toggle').textContent=textMode?'Detailed editor':'One text box';
 detailsPanel.hidden=textMode;questionsPanel.hidden=textMode||(draft.subject==='board-games'&&BOARD_GRADES[draft.format]);
}
...
const oldOpen=openEditor;openEditor=function(...args){if(FORMATS[args[0]?.format]?.standalone)return oldOpen(...args);textMode=args[0]?.subject!=='board-games';pending=false;oldOpen(...args);refresh();};
