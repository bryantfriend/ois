...
const INVESTMENTS={defense:{name:'Defenses',cost:[0,2,1],help:'Protect gold from raids every third round'},army:{name:'Army',cost:[2,0,2],help:'+2 prestige'},farm:{name:'Farm',cost:[2,0,1],help:'+1 wood on each later correct answer'},quarry:{name:'Quarry',cost:[0,2,1],help:'+1 stone on each later correct answer'},monument:{name:'Monument',cost:[2,2,2],help:'+3 prestige'},save:{name:'Save resources',cost:[0,0,0],help:'Save for later rounds or final points'}};
function setupModeEditor(){
 ...
 if(draft.format==='boozled'){host.innerHTML+=`<button type="button" id="boozled-surprise-settings" class="button secondary">🎲 Surprise settings</button><p class="mode-help">${draft.settings.boozledPowerCount??4} surprise tiles · ${esc(draft.settings.boozledMode||'basic')} mode. Choose Basic, All or your own selection before playing.</p>`;$('#boozled-surprise-settings').onclick=()=>openBoozledSurprises(false);}
...
function renderModeGame(){if(game.session?.engine==='activities'){renderActivity();return;}if(game.session?.engine==='expansion'){renderExpansion();return;}
 ...
 if(f==='kingdom'){
  if(s.phase!=='discuss')html=`<div class="mode-round">Teams · Choose your investments</div>`;
  ...
  else html+=`<div class="mode-feedback"><strong>Answer: ${esc(current().answer)}</strong>${current().explanation?'<p>'+esc(current().explanation)+'</p>':''}</div><p class="mode-callout">Strategists: agree one investment per group. Farms and quarries earn extra resources on later correct answers. Monuments earn 3 prestige, armies earn 2. Defenses prevent losing one gold to raids every third round. Every 3 resources left at the end earns 1 point.</p><div class="team-decisions">${s.groups.map((g,i)=>`<section style="--team:${g.color}"><h3>${esc(g.name)} · ${game.choices[g.answer]===current().answer?'Correct':'Miss'}</h3><small>${g.farms} farms · ${g.quarries} quarries · ${g.defenses} defenses · ${g.armies} armies · ${g.prestige} prestige</small>${g.invested?'<p>✓ Investment decided</p>':investmentChooser(g,i)}</section>`).join('')}</div><button class="button primary" data-mode-action="next" ${s.groups.some(g=>!g.invested)?'disabled':''}>${game.index===game.items.length-1?'See kingdoms':'Next round · rotate roles'}</button>`;
 ...
 document.querySelectorAll('[data-invest-pick]').forEach(el=>el.onchange=()=>{const i=Number(el.dataset.investPick);s.groups[i].investPick=el.value;renderGame();$('#investment-'+i)?.focus({preventScroll:true});});
...
function handleModeAction(button){if(game.session?.engine==='activities'){handleActivity(button);return;}if(game.session?.engine==='expansion'){handleExpansion(button);return;}
 ...
 if(action==='lock'&&f==='kingdom'&&s.phase==='discuss'){const i=Number(button.dataset.group),g=s.groups[i];if(!g||g.answer!==null||g.pick===null)return;g.answer=g.pick;}
  ...
  if(f==='kingdom'){
   ...
   let hits=0;s.groups.forEach(g=>{if(game.choices[g.answer]===current().answer){g.resources[0]+=2+g.farms;g.resources[1]+=2+g.quarries;g.resources[2]+=2;hits++;}if((game.index+1)%3===0&&!g.defenses)g.resources[2]=Math.max(0,g.resources[2]-1);});game.attempts+=s.groups.length;game.correct+=hits;game.feedbackGood=hits>0;s.phase='invest';
  }else{
  ...
  v.cost.forEach((n,i)=>g.resources[i]-=n);if(id==='defense')g.defenses++;if(id==='army'){g.armies++;g.prestige+=2;}if(id==='farm')g.farms++;if(id==='quarry')g.quarries++;if(id==='monument')g.prestige+=3;g.invested=true;
  ...
  else{prepareRound();s.phase='discuss';s.choice=null;s.votes=[];s.groups.forEach(g=>{g.answer=null;g.pick=null;g.invested=false;g.investPick="save";});}
 }else return;
 renderGame();
 $('#arena').querySelector('[data-mode-action]:not([disabled]),[data-replay]')?.focus({preventScroll:true});
}
function drawModeWorld(){if(game.session?.engine==='expansion'){drawExpansion();return;}
 if(game.lesson.format==='boozled'){drawBoozledWorld();return;}
...
function investmentChooser(g,i){const id=g.investPick||'save',v=INVESTMENTS[id];return `<label>Investment<select id="investment-${i}" data-invest-pick="${i}">${Object.entries(INVESTMENTS).map(([key,item])=>`<option value="${key}" ${key===id?'selected':''} ${item.cost.some((cost,n)=>g.resources[n]<cost)?'disabled':''}>${item.name}</option>`).join('')}</select></label><p class="investment-detail">🪵 ${v.cost[0]} · 🪨 ${v.cost[1]} · 🪙 ${v.cost[2]}<br>${v.help}</p><button class="button" data-mode-action="invest" data-group="${i}" data-investment="${id}" ${v.cost.some((cost,n)=>g.resources[n]<cost)?'disabled':''}>Confirm</button>`;}
