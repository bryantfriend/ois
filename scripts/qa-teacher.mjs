...
try{
 ...
 assert.equal(await page.evaluate(()=>GAMES.map(validateLesson).length),40);
 ...
 assert.equal(toolResult.name,'filter_classroom_games');assert.equal(toolResult.result.summary,'10 lessons · Grade 8');assert.equal(toolResult.visible,'10 lessons · Grade 8');assert.equal(toolResult.invalid,true);await toolsContext.close();
